    ...
    <div className="min-h-screen bg-[#050505] relative overflow-hidden">
      ...
      <main className="relative z-10 pt-44 pb-32 px-6 md:px-12 lg:px-20">
        <div className="max-w-[1100px] mx-auto">
          {/* Título Principal Idêntico */}
          <div className="text-center mb-20">
            <h1 className="text-5xl md:text-7xl font-bold text-white tracking-[25px] md:tracking-[40px] uppercase font-space mb-6 ml-[25px] md:ml-[40px]">
              SETUP
            </h1>
            <div className="w-20 h-[3px] bg-primary mx-auto rounded-full shadow-[0_0_20px_rgba(255,70,85,0.6)]" />
          </div>
            ...
            {setupSections.map((section, idx) => (
              <section key={idx} className="page-transition">
                ...
                <div className="flex items-center gap-3 mb-8 ml-4">
                  ...
                  <h2 className="text-[10px] font-bold uppercase tracking-[8px] text-white/20 font-space">
                    {section.category}
                  </h2>
                </div>
                
                {/* Grid de Cards Idêntico ao Site */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {section.items.map((item, itemIdx) => (
                    <div 
                      key={itemIdx} 
                      className="group flex flex-col bg-[#0a0a0a] border border-white/[0.04] rounded-[28px] p-2 transition-all duration-500 hover:border-primary/30 hover:bg-[#0d0d0d] hover:-translate-y-2 shadow-2xl"
                    >
                      {item.image ? (
                        <div className="aspect-[1.3/1] w-full flex items-center justify-center p-8 relative bg-[#121212]/50 border border-white/[0.03] rounded-[22px] overflow-hidden">
                          <img 
                            src={item.image} 
                            alt={item.name}
                            className="max-h-full max-w-full object-contain transition-transform duration-700 group-hover:scale-110 z-10 drop-shadow-[0_12px_24px_rgba(0,0,0,0.8)]"
                          />
                        </div>
                      ) : (
                      ...
                      <div className="px-5 py-5 flex flex-col items-center text-center">
                        <h3 className="text-white text-[13px] font-bold mb-1 tracking-wide group-hover:text-primary transition-colors duration-300">
                          {item.name}
                        </h3>
                        <p className="text-white/30 text-[10px] font-bold uppercase tracking-[2px] truncate w-full">
                          {item.spec}
                        </p>
                      </div>
                    </div>
